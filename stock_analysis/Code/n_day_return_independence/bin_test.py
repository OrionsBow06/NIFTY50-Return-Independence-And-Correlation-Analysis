...
def main(n):
    ...
    for symbol in companies["Symbol"]:
        return_data = pd.read_csv(data_path / f"derived_data/{num}_day_return_segregation_data"
                                  / f"{symbol}_segregated_{num}_day_return_data.csv")
        test_process(symbol, return_data)
    print("***Test Complete***")

    df = pd.DataFrame(test_result)
    save_frame(df, f"{num}_day_bin_test_result_raw.csv")
    df = process_data(df)
    print(df)
    save_frame(df, f"{num}_day_bin_test_result_processed.csv")
    summary = prepare_summary(df)
    save_frame(summary, f"{num}_day_bin_test_result_summary.csv")
